...
function init(array &$dadosboleto)
{
    ...
    $codigocliente = $this->formata_numero($dadosboleto["codigo_cedente"],11,0,"valor");
    ...
    $vencjuliano = $this->dataJuliano($dadosboleto["data_vencimento"]);
    
    // Calcula Campo Livre
    $campoLivre = $this->calculaCampoLivre($codigocliente.$nossonumero."00".$codigobanco);
    ...
    $linha = $this->monta_codigo_de_barras($codigobanco.$nummoeda.$fator_vencimento.$valor.$codigocliente.$nossonumero."00".$codigobanco.substr($campoLivre, strlen($campoLivre)-2));
    
    $dadosboleto["codigo_barras"] = $linha;
    $dadosboleto["linha_digitavel"] = $this->monta_linha_digitavel($linha);
    $dadosboleto["nosso_numero"] = $this->calcula_verificador_nosso_numero($dadosboleto["ponto_venda"], $nossonumero);
...
function geraNossoNumero($no_pedido,$dig_inicial) 
{
    ...
    return $ndoc . $this->modulo_11($ndoc,9,0);
}
...
function calculaCampoLivre ($num) {
    ...
    $digitoUm   = $this->modulo_10($num);
    $digitoDois = 1;

    $loop=0;
    while ($digitoDois == 1) {
        $digitoDois = $this->modulo_11($num.$digitoUm);
        if ($recalcular=="N"){
            break;
        }
    }
...
function monta_linha_digitavel($linha){
    ...
    $campo1Digito = $this->modulo_10($campo1);
    
    $campo2 = substr($linha,24,10);
    $campo2Digito = $this->modulo_10($campo2);
    
    $campo3 = substr($linha,34,10);
    $campo3Digito = $this->modulo_10($campo3);

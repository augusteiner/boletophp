...
//conta é 7 digitos
$conta    = $this->formata_numero($dadosboleto["conta"], 7, 0);
//nosso número: 13 digitos
$nossonum = $this->formata_numero($dadosboleto["nosso_numero"], 13, 0);

// Calcula o Digitão da Cobrança
$digitao = $this->modulo_10($nossonum . $agencia . $conta);
if ($digitao == 10) $digitao = 0;

// 43 numeros para o calculo do digito verificador do codigo de barras
$dv = $this->modulo_11($codigobanco . $nummoeda . $fator_vencimento . $valor . $agencia . $conta . $digitao . $nossonum, 9, 1);
...
$dadosboleto["linha_digitavel"]     = $this->monta_linha_digitavel($codigobanco . $nummoeda . $agencia . $conta . $digitao . $nossonum . $dv . $fator_vencimento . $valor);
...
function monta_linha_digitavel($dados) {
    ...
    if (preg_match('/^([0-9]{5})([0-9]{4})([0-9]{5})([0-9]{5})([0-9]{5})([0-9]{5})([0-9]{1})([0-9]{4})([0-9]{10})$/', $dados, $bloco))
        {
        $dv1 = $this->modulo_10($bloco[1] . $bloco[2]);
        if ($dv1 == 10) $dv1 = 0;

        $dv2 = $this->modulo_10($bloco[3] . $bloco[4]);
        if ($dv2 == 10) $dv2 = 0;

        $dv3 = $this->modulo_10($bloco[5] . $bloco[6]);
        if ($dv3 == 10) $dv3 = 0;

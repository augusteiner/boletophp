...
$codigocedente = $this->formata_numero($dadosboleto["codigo_cedente"],5,0);

$ndoc = $dadosboleto["numero_documento"];
$vencimento = $dadosboleto["data_vencimento"];

// número do documento (sem dvs) é 13 digitos
$nnum = $this->formata_numero($dadosboleto["nosso_numero"],13,0);
// nosso número (com dvs) é 16 digitos
$nossonumero = $this->geraNossoNumero($nnum);
...
$dv1 = $this->m10dv1($chave);
$dv2 = $this->m10dv2($chave . $dv1);
$barra = $barra . $dv1 . $dv2;
...
function geraNossoNumero($ndoc) {
    return $ndoc."-".$this->digitoVerificador_nossonumero($ndoc);
}
...
function digitoVerificador_nossonumero($numero) {
    $dv = $this->modulo_11_invertido($numero);
    $dv = $dv.'3';
    $dv = $dv.$this->modulo_11_invertido($numero.$dv);
    return $dv;
}
...
function monta_linha_digitavel($codigo) {
    ...
    $campo4 = $this->modulo_10_geral(substr($codigo, 0, 4).substr($codigo, 5, 49));
